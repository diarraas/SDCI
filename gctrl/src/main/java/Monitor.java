...
class Monitor {
    ...
    private void symptom_generator() {
        while (Main.run)
            try {
                ...
                double current_load = get_fake_data();
                Main.logger(this.getClass().getSimpleName(), "Current cpu load : : " + current_load);
                boolean isOk = true;
                if (current_load > Knowledge.gw_lat_threshold) {
            ...
            } catch (Exception e) {//(SQLException | InterruptedException e) {
                e.printStackTrace();
            }
    }
    ...
    public int get_data() {
    	...
    	int load = -1;
    	try{
    		URL url = new URL("http://172.17.0.3:8181/health");
            ...
            load = (int) jObject.getFloat("currentload");
            http.disconnect();
    	}catch (Exception e){
    		e.printStackTrace();
    	}
